...
- **Не вешать `display:none` / unmount неба на `max-width` и не размонтировать небо на `pointer: coarse`.** Иначе на телефоне нет луны и после гидрации прыгает герой. Lite: только swell/звёзды через CSS. На мобилке прятать `.lift-deck` (четыре кружка между героем и «Сейчас свободно»), на десктопе колоду оставить.
